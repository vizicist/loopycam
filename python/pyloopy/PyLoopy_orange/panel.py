...
class PresetSetValue(NthValue):
	...
	def __init__(self, panel):
		NthValue.__init__(self, panel, default="orange")
		self.files = ["orange", "tedx", "new", "good", "good_noswrl", "abstract", "basic", "subtle" ]
...
class NthControlPanel(NthEventProcessor):
	...
	def set_all_random_params_of_all_plugins(self, d1, d2, random_amount=1.0):
		...
		# hack for Rick Walker's orange gig - don't randomize the last FFGL plugin,
		# which is the one that forces the orange color
		for n in range(NFFGL-1):
			b = self.butt["ffgl"][n]
			b.set_bgcolor(self.bgcolor)
			if b.text != "None":
	...
	def load_clear_preset(self):
		fname = self.preset_fullpath("orange", "clear")
		try:
			self.read_preset(fname)
		except:
			print "Unable to load fname=", fname
