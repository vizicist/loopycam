...
TCP::cSocket::cSocket()
{
	...
	ms64_MaxIdleTime  = 0;
	mu32_EventTimeout = 0;
	// NS_debug("TCP::cSocket::cSocket  CONSTRUCTED\n");
}

TCP::cSocket::~cSocket()
{
	// NS_debug("TCP::cSocket::cSocket  DESTRUCTOR\n");
	if (mb_Initialized) 
	{
		Close();
		WSACleanup();
...
{
	// NS_debug("TCP::cSocket::cSocket  Initialize\n");
	if (mb_Initialized)
		return 0;
...
{
	// NS_debug("TCP::cSocket::cSocket  Close\n");
...
{
	// NS_debug("TCP::cSocket::cSocket  Listen\n");
	#if TRACE_EVENTS || TRACE_LOCK
		TraceA("Listen()");
	#endif
...
{
	// NS_debug("TCP::cSocket::cSocket  ConnectTo  ServIP=0x%lx\n",u32_ServIP);
	#if TRACE_EVENTS || TRACE_LOCK
		TraceA("ConnectTo()");
	#endif
...
{
	// NS_debug("TCP::cSocket::cSocket  DisconnectClient\n");
	#if TRACE_EVENTS || TRACE_LOCK
...
DWORD TCP::cSocket::ProcessEvents(DWORD*   pu32_Events,  // OUT
								  ...
								  DWORD*   pu32_Sent)    // OUT
{
	// NS_debug("TCP::cSocket::cSocket  ProcessEvents\n");
	#if TRACE_EVENTS || TRACE_LOCK
	...
	*ppi_RecvMem = 0;
	...
	if (!mi_List.mu32_Count) {
		NS_debug("Returning WSAENOTCONN, mu32_count = 0?\n");
		return WSAENOTCONN; // No socket open
	}
	...
	*pu32_IP   = pk_Data->u32_IP;
	...
	if (k_Events.lNetworkEvents & FD_CONNECT)
	{
		if (k_Events.iErrorCode[FD_CONNECT_BIT])
		{
			...
			NS_debug("cSocket event FD_CONNECT error = %d\n",u32_Error);
			goto _Exit;
		}

		// Store the time when the last action was executed on the socket
		pk_Data->s64_IdleSince = GetTickCount64();
		NS_debug("cSocket event FD_CONNECT good!\n");
